...
`timescale 1ns/1ps // FIX #1: added missing timescale directive
...
// `include "fifo_memory.sv"
 ...
 module asynchronous_fifo #(parameter FIFO_DEPTH=8, FIFO_WIDTH=64) (
   ...
   reg [PTR_WIDTH:0] g_wptr, g_rptr; // FIX #2: trailing comma changed to semicolon

   //binary write and read pointer 
   reg [PTR_WIDTH:0] b_wptr, b_rptr;
  

   wire [PTR_WIDTH-1:0] waddr, raddr; // FIX #3: added missing semicolon
   ...
   rptr_handler #(PTR_WIDTH) rdptr_h   (
					...
					.b_rptr		(b_rptr),
					.g_rptr		(g_rptr),
					.fifo_empty	(fifo_empty) // FIX #4: added missing dot prefix
				       );

  fifo_mem #(FIFO_DEPTH, FIFO_WIDTH, PTR_WIDTH) fifom ( // FIX #5: was 'fifo_memory fifom', corrected module name and added parameters
					.wrclk		(wrclk), // FIX #6: added missing dot prefix
					.wr_en		(wr_en),
				        .rdclk		(rdclk), 
					.rd_en		(rd_en),
					.b_wptr		(b_wptr), 
					...
					.data_out	(data_out) // FIX #7: removed trailing comma
				       );
